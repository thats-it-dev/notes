 ...
 */
async function refreshAccessToken(syncUrl: string, refreshToken: string): Promise<{ accessToken: string; refreshToken: string } | null> {
  try {
    ...
    if (syncUrl && refreshToken) {
      ...
      refreshAccessToken(syncUrl, refreshToken).then((tokens) => {
        if (tokens) {
          ...
          syncEngine.start(30000); // Sync every 30s
          setIsEnabled(true);
        } else {
